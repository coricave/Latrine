...
if (!is_dir($store)) {
    if (!mkdir($store)) {
        die("Úložiště není k dispozici");
    }
}

@$spojeni=mysqli_Connect($server, $login, $pass, $database);
if (!$spojeni){
   echo "<center><font size=5>Chyba při připojování k serveru... <br><br><a href=javascript:history.back(-1)>Zpět</a>";
}
...
#	}
	##### Write to file
        ...
        if ($handle = fopen($store, "w")) {
            ...
            fclose($handle);
            $zapis=mysqli_query($spojeni, "insert into Poloha values (NULL,'$lat','$lon','$alt','$speed','$bearing','$key','$acc','$battery','$gsm_signal');");
        } else {
